...
struct TelemetrySample {
    ...
    uint64_t lowcmd_steady_time_ns = 0;
    uint64_t lowcmd_sequence = 0;
    bool lowcmd_received = false;
    uint8_t lowcmd_mode_pr = 0;
    uint8_t lowcmd_mode_machine = 0;
    std::array<uint32_t, 4> lowcmd_reserve{};
    uint32_t lowcmd_crc = 0;
    ...
    std::array<uint8_t, kMaxMotorCount> cmd_mode{};
    std::array<float, kMaxMotorCount> cmd_q{};
    std::array<float, kMaxMotorCount> cmd_dq{};
    std::array<float, kMaxMotorCount> cmd_tau{};
    std::array<float, kMaxMotorCount> cmd_kp{};
    std::array<float, kMaxMotorCount> cmd_kd{};
    std::array<uint32_t, kMaxMotorCount> cmd_reserve{};
